...
module.exports = async (req, res) => {
    ...
    if (req.method === 'GET') {
        return res.status(200).json({
            status: 'FM Assist V2 API Online',
            project: projectHost,
            supabase: !!supabase,
            config_ready: !!process.env.CHAT_WIDGET_SECRET
        });
    }

    if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

    console.log(`Chat API [${projectHost}]: Incoming request`);

    const secret = process.env.CHAT_WIDGET_SECRET || 'FM_ASSIST_SECRET';
    if (req.headers['x-fm-secret'] !== secret) {
        console.error(`Chat API [${projectHost}]: Unauthorized - Secret mismatch`);
